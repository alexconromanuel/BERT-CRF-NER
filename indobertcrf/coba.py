...
def run(dataset):
    ...
    optimizer = AdamW(model.parameters(),
                      lr=learning_rate0, correct_bias=False)

    total_train_epochs = 5
    gradient_accumulation_steps = 1
    total_train_steps = int(len(train_examples) / batch_size /
                            gradient_accumulation_steps * total_train_epochs)
    global_step_th = 0
    ...
    for epoch in range(total_train_epochs):
        ...
        for step, batch in tqdm(enumerate(train_dataloader)):
            ...
            if (step + 1) % gradient_accumulation_steps == 0:
                ...
                lr_this_step = learning_rate0 * \
                    ner.warmup_linear(
                        global_step_th/total_train_steps, warmup_proportion)
        ...
        print('--------------------------------------------------------------')
        print("Epoch:{} completed, Total training's Loss: {}, Spend: {}m".format(
            epoch, tr_loss, (time.time() - train_start)/60.0))
